...
const Wishlist = () => {
    ...
    useEffect(() => {
        const fetchWishlistItems = async () => {
            try {
                const response = await fetch(`http://localhost:4000/api/wishlist/${userId}`);
    ...
    const removeProductFromwishlist = async (productId) => {
        ...
        try {
             await axios.put(`http://localhost:4000/api/wishlist/add/${userId}`, { product: productId });
            setWishlistItems(prevItems => prevItems.filter(product => product !== productId));
        } catch (error) {
    ...
    const addToCart = async (pId) => {
        try {
            ...
            const response = await axios.put(`http://localhost:4000/api/cart/add/${userId}`, { product: pId });
                            ...
                            <div key={productId} className="cupcakeDiv col-3 col-t-6 col-m-6">
                                {productData[productId] && (
                                    ...
                                    isProductInCart(productId)?(
                                        <NavLink to={"/cart"} ><button className="link"  type="submit">Go to cart</button><br /></NavLink>
                                    ):(
                                        <><button className="link" onClick={() => addToCart(productId)} type="submit">Add to cart</button><br /></>
                                    )
                                }
                                    <button className="link" type="submit" onClick={() => { removeProductFromwishlist(productId) }}>Remove</button>
                                    </div>
                                )}
                            </div>
                        ))}
