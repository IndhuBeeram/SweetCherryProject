...
const ProductDetails = () => {
    ...
    const addToWishlist = async (pId) => {
        try {
            ...
            const response = await axios.put(`http://localhost:4000/api/wishlist/add/${userId}`, { product: pId });
                            ...
                            <div className="cupcakeTextV">
                                ...
                                <em>Rs. {data.price}</em>
                                {
                                    isProductInCart(data._id)?(
                                        <NavLink to={"/cart"}><button  type="submit">Go to cart</button><br /></NavLink>
                                    ):(
                                ...
                                {isProductInWishlist(data._id) ? (
                                    <button onClick={() => removeFromWishlist(data._id)} className="btn btn-danger">Remove from Wishlist</button>
                                ) : (
                                    <button onClick={() => addToWishlist(data._id)} className="btn btn-success">Add to Wishlist</button>
                                )}
                            </div>
                        </div>
                    </div>
